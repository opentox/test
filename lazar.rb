...
def test_create_regression_svm_pc_model
  ...
  assert_in_delta @predictions.first.value(@compounds.first), 6.82, 0.3
  assert_equal 0.761, @predictions.first.confidence(@compounds.first).round_to(3)
  assert_equal 62, @predictions.first.neighbors(@compounds.first).size
  cleanup
end
